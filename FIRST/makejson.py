	...
	for row in csvReader:
		if len(row) == 1:
			...
			if row != []:
				...
				schedule["matches"].append({"name": "Q" + row[1], "redAlliance": [{"number": row[2], "name": '-'}, {"number": row[3], "name": '-'}, {"number": row[4], "name": '-'}], "blueAlliance": [{"number": row[5], "name": bt1rr}, {"number": row[6], "name": '-'}, {"number": row[7], "name": '-'}]})
